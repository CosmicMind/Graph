...
public class GKSet<K: Comparable, V>: Printable {
	...
	public func search(key: K) -> GKSet<K, V> {
		var s: GKSet<K, V> = GKSet<K, V>()
		traverse(key, node: tree.root, set: &s)
		return s
	}
	
	private func traverse(key: K, node: GKRedBlackNode<K, V>, inout set: GKSet<K, V>) {
