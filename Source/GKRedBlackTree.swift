...
internal class GKRedBlackTree<K: Comparable, V>: Printable {
	...
	internal func select(var x: GKRBNode, order: Int) -> GKRBNode {
	...
	internal subscript(index: Int) -> V? {
		get {
			if index < 0 || index >= count {
				assert(false, "[GKAlgoKit Error: Index out of bounds.]")
			}
	...
	internal subscript(name: String) -> V? {
		...
		set(value) {
			let node: GKRBNode = internalFindByKey(name as! K)
			if sentinel === node {
	...
	private func internalInsert(key: K, data: V?) -> GKRBNode {
		var y: GKRBNode = sentinel
		var x: GKRBNode = root
		...
		var z: GKRBNode = GKRBNode(parent: y, sentinel: sentinel, key: key, data: data)
	...
	private func insertCleanUp(var z: GKRBNode) {
		while true == z.parent.red {
			if z.parent === z.parent.parent.left {
				var y: GKRBNode = z.parent.parent.right
				...
				// symetric
				var y: GKRBNode = z.parent.parent.left
				// violation 1, parent child relationship re to red
				if true == y.red {
					z.parent.red = false
					y.red = false
	...
	private func internalRemove(key: K) -> GKRBNode {
		var z: GKRBNode = internalFindByKey(key)
		if z === sentinel {
			return sentinel
		}
		
		if z !== root {
			var t: GKRBNode = z.parent
			while t !== root {
				--t.order
				t = t.parent
			}
	...
	private func removeCleanup(var x: GKRBNode) {
		while x !== root && false == x.red {
			if x === x.parent.left {
				var y: GKRBNode = x.parent.right
				if true == y.red {
					y.red = false
					x.parent.red = true
					leftRotate(x.parent)
	...
	private func minimum(var x: GKRBNode) -> GKRBNode {
		var y: GKRBNode = sentinel
		while x !== sentinel {
			y = x
			x = x.left
	...
	private func internalFindByKey(key: K) -> GKRBNode {
		var z: GKRBNode = root
		while z !== sentinel {
			if key == z.key {
				return z
